...
router.get('/page/:page', function(req, res, next) {
  ...
  Mission.find(searchQuery).sort('-createdAt').populate({path: 'projects', model: 'Project'}).populate({path: 'users', model: 'User'}).populate({path: 'categories', model: 'Categorie'})
  ...
  //   })
    ...
    if (err) {
      ...
      Mission.find(searchQuery).count().exec(function(err, count) {
...
router.get('/:id', function(req, res, next) {
  ...
  Mission.findById((req.params.id), function(err, obj) {
    ...
    Mission.findById({_id: req.params.id}).populate({path: 'projects', model: 'Project'}).populate({path: 'categories', model: 'Categorie'}).populate({path: 'users', model: 'User'}).populate({
      path: 'users',
      model: 'User',
      populate: {
        path: 'forms',
